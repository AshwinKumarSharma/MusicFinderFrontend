...
import { analyzeTrackVibe, calculateVibeCompatibility, generateVibeSearchQueries } from './vibe-analyzer';
...
export class DJController {
  ...
  private saveState() {
    try {
      localStorage.setItem('dj-state', JSON.stringify({
    ...
    try {
      ...
      for (const query of mode.searchQueries) {
        try {
          ...
          console.log(`📀 Found ${searchTracks.length} tracks for mode query: "${query}"`);
      ...
      console.log(`✅ DJ Mode Queue Built: ${filteredTracks.length} tracks for "${mode.name}"`);
      ...
      if (filteredTracks.length > 0) {
        ...
        console.log(`🎵 Started playing: "${initialTrack.trackName}" by ${initialTrack.artistName}`);
      } else {
  ...
  private filterTracksByMode(
    ...
    const uniqueTracks = tracks.filter((track, index, arr) => {
      return !excludeTrackIds.includes(track.trackId) &&
    ...
    const rankedTracks = uniqueTracks.map(track => {
      ...
      if (detectedLanguage === mode.language) {
        ...
        console.log(`🎯 MODE LANGUAGE MATCH: "${track.trackName}" - ${detectedLanguage} === ${mode.language}`);
      } else {
        score -= 0.3; // Heavy penalty for wrong language
        console.log(`❌ MODE LANGUAGE MISMATCH: "${track.trackName}" - ${detectedLanguage} ≠ ${mode.language}`);
      }
      
      // Genre matching (20% weight)
      if (track.primaryGenreName && mode.genres.includes(track.primaryGenreName)) {
        score += 0.2; // Perfect genre match for mode
        console.log(`🎵 MODE GENRE MATCH: "${track.trackName}" - ${track.primaryGenreName}`);
      }
    ...
    const sortedTracks = rankedTracks
      ...
      .map(item => {
        console.log(`📊 Mode Score: ${item.score.toFixed(3)} - "${item.track.trackName}" (${item.detectedLanguage})`);
        return item.track;
      });
  ...
  markTrackPlayed(track: Track): void {
    ...
    const updatedNextTracks = this.state.nextTracks.filter(t => t.trackId !== track.trackId);
    ...
    if (this.state.autoQueue && updatedNextTracks.length < 3 && this.state.currentVibeProfile) {
  ...
  async addTrackToQueue(track: Track): Promise<void> {
    ...
    if (this.state.currentVibeProfile) {
      const newVibeProfile = this.blendVibeProfiles(this.state.currentVibeProfile, trackVibe);
      this.setState({
  ...
  removeTrackFromQueue(trackId: number): void {
    const updatedNextTracks = this.state.nextTracks.filter(t => t.trackId !== trackId);
    this.setState({
      nextTracks: updatedNextTracks,
    });
  }
  ...
  // Get queue status
  getQueueStatus(): { nextCount: number; playedCount: number; vibeProfile: VibeProfile | null } {
    return {
  ...
  private async buildQueue(vibeProfile: VibeProfile, excludeTrackIds: number[]): Promise<void> {
    try {
      // Get reference track for smart matching
      const referenceTrack = this.state.playedTracks[this.state.playedTracks.length - 1];
      
      if (referenceTrack) {
        // Use smart matching based on primaryGenreName, country, and duration
        const smartTracks = await this.buildSmartQueue(referenceTrack, excludeTrackIds);
  ...
  private async buildSmartQueue(referenceTrack: Track, excludeTrackIds: number[]): Promise<Track[]> {
    ...
    try {
      ...
      if (targetLanguage) {
        const languageQueries = this.buildLanguageSpecificQueries(targetLanguage, targetGenre);
        console.log(`🔍 LANGUAGE-BASED Search Queries (TOP PRIORITY):`, languageQueries);
        
        for (const query of languageQueries) {
          try {
            console.log(`🔍 Searching LANGUAGE: "${query}"`);
            const tracks = await this.searchTracks(query);
            console.log(`📀 Found ${tracks.length} LANGUAGE tracks for: "${query}"`);
      ...
      if (smartTracks.length < 20 && targetGenre) {
        console.log(`⚠️ Only found ${smartTracks.length} language tracks, adding genre-based tracks...`);
        const genreQueries = this.buildGenreSpecificQueries(targetGenre, targetCountry);
        
        for (const query of genreQueries.slice(0, 3)) {
          try {
            const tracks = await this.searchTracks(query);
            console.log(`📀 Found ${tracks.length} genre tracks for: "${query}"`);
            smartTracks.push(...tracks);
            if (smartTracks.length >= 50) break;
          } catch (error) {
      ...
      console.log(`📊 Total tracks found: ${smartTracks.length}, now filtering with LANGUAGE PRIORITY...`);
      ...
      console.log(`✅ Smart Queue Built: ${filteredTracks.length} tracks selected with LANGUAGE PRIORITY`);
  ...
  private buildGenreSpecificQueries(targetGenre: string, targetCountry?: string): string[] {
    ...
    const genreSearchMap: Record<string, string[]> = {
      ...
      'Bollywood': ['bollywood songs', 'hindi songs', 'bollywood music', 'hindi film songs'],
      ...
      'Electronic': ['electronic music', 'edm', 'dance music', 'electronic dance'],
    ...
    const searchTerms = genreSearchMap[targetGenre] || [targetGenre.toLowerCase()];
  ...
  private isRegionalGenre(genre: string): boolean {
    const regionalGenres = [
      'Punjabi', 'Bollywood', 'Tamil', 'Telugu', 'Malayalam', 'Kannada', 'Marathi', 
      'Bengali', 'Gujarati', 'K-Pop', 'J-Pop', 'Latin', 'Reggae', 'Country'
    ];
  ...
  private detectTrackLanguage(track: Track): string {
    const trackText = `${track.trackName} ${track.artistName} ${track.collectionName || ''}`.toLowerCase();
    ...
    for (const [language, patterns] of Object.entries(languagePatterns)) {
      for (const pattern of patterns) {
        if (pattern.test(trackText)) {
          console.log(`🌍 LANGUAGE DETECTED: "${language}" for track "${track.trackName}" (pattern matched: ${pattern})`);
          return language;
        }
      }
    }
    ...
    if (track.primaryGenreName && genreLanguageMap[track.primaryGenreName]) {
      ...
      console.log(`🌍 LANGUAGE DETECTED from genre: "${detectedLang}" for genre "${track.primaryGenreName}"`);
      return detectedLang;
    }

    console.log(`⚠️ NO LANGUAGE DETECTED for track "${track.trackName}" - defaulting to 'english'`);
    return 'english'; // Default fallback
  }

  // Build language-specific search queries (HIGHEST PRIORITY)
  private buildLanguageSpecificQueries(language: string, genre?: string): string[] {
    const queries: string[] = [];
    
    const languageQueryMap: Record<string, string[]> = {
      'punjabi': ['punjabi songs', 'punjabi music', 'bhangra', 'punjabi hits', 'desi punjabi', 'jatt songs'],
      'hindi': ['hindi songs', 'bollywood music', 'hindi film songs', 'bollywood hits', 'desi songs', 'filmi songs'],
      'tamil': ['tamil songs', 'kollywood music', 'tamil film songs', 'tamil hits', 'chennai music'],
      'telugu': ['telugu songs', 'tollywood music', 'telugu film songs', 'telugu hits', 'andhra music'],
      'malayalam': ['malayalam songs', 'mollywood music', 'malayalam film songs', 'kerala music'],
      'kannada': ['kannada songs', 'sandalwood music', 'kannada film songs', 'karnataka music'],
      'marathi': ['marathi songs', 'marathi music', 'marathi film songs', 'maharashtra music'],
      'bengali': ['bengali songs', 'bangla music', 'bengali film songs', 'kolkata music'],
      'gujarati': ['gujarati songs', 'gujarati music', 'gujarat music'],
      'korean': ['kpop', 'k-pop', 'korean pop', 'korean music', 'hallyu'],
      'japanese': ['jpop', 'j-pop', 'japanese pop', 'japanese music', 'anime songs'],
      'english': ['pop songs', 'english music', 'american music', 'british music', 'western music']
    };
  ...
  private filterByLanguagePriority(
    ...
    const rankedTracks = uniqueTracks.map(track => {
      ...
      if (detectedLanguage === targetLanguage) {
        ...
        console.log(`🎯 PERFECT LANGUAGE MATCH: "${track.trackName}" - ${detectedLanguage} === ${targetLanguage}`);
      } else {
        ...
        if (compatibleLanguages.includes(detectedLanguage)) {
          ...
          console.log(`🎯 COMPATIBLE LANGUAGE: "${track.trackName}" - ${detectedLanguage} ~ ${targetLanguage}`);
        } else {
          score -= 0.5; // Heavy penalty for different language to prevent English mixing with Punjabi
          console.log(`❌ LANGUAGE MISMATCH: "${track.trackName}" - ${detectedLanguage} ≠ ${targetLanguage}`);
        }
      }
      ...
      if (targetCountry && track.country) {
        if (track.country === targetCountry) {
          ...
          const compatibleCountries = this.getCompatibleCountries(targetCountry);
      ...
      if (durationRange && track.trackTimeMillis) {
        if (track.trackTimeMillis >= durationRange.min && track.trackTimeMillis <= durationRange.max) {
    ...
    const sortedTracks = rankedTracks
      .sort((a, b) => b.score - a.score)
      .map(item => {
        console.log(`📊 Final Score: ${item.score.toFixed(3)} - "${item.track.trackName}" (${item.detectedLanguage})`);
        return item.track;
      });
  ...
  private filterAndRankBySmartCriteria(
    ...
    const rankedTracks = uniqueTracks.map(track => {
      ...
      if (targetGenre && track.primaryGenreName) {
        ...
        } else if (this.getSimilarGenres(targetGenre).includes(track.primaryGenreName)) {
      ...
      if (minDuration && maxDuration && track.trackTimeMillis) {
        if (track.trackTimeMillis >= minDuration && track.trackTimeMillis <= maxDuration) {
          ...
          const penalty = Math.min(0.15, (durationDiff / targetDuration) * 0.15);
          score -= penalty;
        }
      }
  ...
  private getCompatibleCountries(country: string): string[] {
    const countryMap: Record<string, string[]> = {
      ...
      'United Kingdom': ['Ireland', 'Australia', 'New Zealand', 'Canada', 'USA'],
  ...
  private getConflictingGenres(genre: string): string[] {
    const conflictMap: Record<string, string[]> = {
      'bollywood': ['new age', 'meditation', 'ambient', 'devotional', 'spiritual', 'relaxation'],
      ...
      'hip-hop/rap': ['meditation', 'new age', 'devotional', 'spiritual', 'classical'],
  ...
  private isHighEnergyTrack(track: Track): boolean {
    ...
    const highEnergyKeywords = [
      'dance', 'party', 'celebration', 'festival', 'beat', 'pump', 'energy', 'power',
      'rock', 'metal', 'electronic', 'edm', 'remix', 'club', 'rave', 'bounce',
      'thumka', 'nachle', 'dance', 'party', 'dj', 'bass', 'drop', 'bangers'
    ];
    
    // Low energy keywords that indicate calm/meditative tracks
    const lowEnergyKeywords = [
      'meditation', 'peace', 'calm', 'relax', 'spiritual', 'devotional', 'prayer',
      'ambient', 'sleep', 'healing', 'therapy', 'yoga', 'mindfulness', 'zen',
      'soulful', 'peaceful', 'gentle', 'soft', 'whisper', 'lullaby'
    ];
    ...
    const highEnergyGenres = ['dance', 'electronic', 'hip-hop', 'rap', 'rock', 'pop', 'bollywood', 'punjabi', 'bhangra'];
    const lowEnergyGenres = ['meditation', 'new age', 'ambient', 'classical', 'devotional', 'spiritual'];
  ...
  private filterAndRankTracks(
    ...
    const rankedTracks = uniqueTracks.map(track => {
      ...
      if (targetVibe.filmIndustry && trackVibe.filmIndustry) {
        if (targetVibe.filmIndustry === trackVibe.filmIndustry) {
        ...
        } else if (targetVibe.filmIndustry !== 'other' && trackVibe.filmIndustry !== 'other') {
          ...
          if (compatibleIndustries[targetVibe.filmIndustry]?.includes(trackVibe.filmIndustry)) {
      ...
      if (targetVibe.language && trackVibe.language) {
        ...
        } else if (targetVibe.language !== 'other' && trackVibe.language !== 'other') {
      ...
      const finalScore = Math.max(0, Math.min(1, compatibility + filmIndustryBoost + languageBoost + genreBoost));
  ...
  private blendVibeProfiles(current: VibeProfile, new_: VibeProfile): VibeProfile {
  ...
  private blendEnergy(current?: string, new_?: string): 'low' | 'medium' | 'high' | undefined {
    ...
    const energyKeys = Object.keys(energyLevels) as Array<keyof typeof energyLevels>;
    return energyKeys[blended - 1];
  }

  private blendMood(current?: string, new_?: string): 'happy' | 'sad' | 'energetic' | 'relaxed' | 'party' | 'chill' | undefined {
    if (!current || !new_) return (current as any) || (new_ as any);
    ...
    if (compatibleMoods[current]?.includes(new_)) {
      return current as 'happy' | 'sad' | 'energetic' | 'relaxed' | 'party' | 'chill'; // Keep current mood if compatible
    }
    
    return new_ as 'happy' | 'sad' | 'energetic' | 'relaxed' | 'party' | 'chill'; // Adapt to new mood if not compatible
  }

  private blendTempo(current?: string, new_?: string): 'slow' | 'medium' | 'fast' | undefined {
    if (!current || !new_) return current as any || new_ as any;
    ...
    const blended = Math.round((currentLevel * 0.7) + (newLevel * 0.3));
    const tempoKeys = Object.keys(tempoLevels) as Array<keyof typeof tempoLevels>;
